...
<div class="container">
    <div class="toolbar">
        <div class="btn-group" style="margin-left: 48rem">
        ...
        <table id="tabla_productos" class="table table-hover" data-search="true" data-custom-search="customSearch" data-pagination="true" data-url="{{ route('listar_productos') }}" data-height="550">
            <thead class="thead-dark">
                <tr class="tipo_letra_encabezado">
                    ...
                    <th data-field="regular_price" data-formatter="regular_priceFormatter" scope="col">Precio Regular
                    </th>
                    <th data-field="sale_price" data-formatter="sale_priceFormatter" scope="col">Precio Venta</th>
                    <th data-field="Acciones" data-formatter="accionesFormatter" data-events="accionesEvent" scope="col"
                        data-width="111">Acciones</th>
                </tr>
            </thead>
            <tbody>
...
<script>
    ...
    function customSearch(data, text) {
        return data.filter(function (row) {
            var $campo=$('[name="filterAlgorithm"]').val()
            switch ($campo) {
                case 'code':
                return row.code.toString().toUpperCase().startsWith(text.toString().toUpperCase())
                    break;
                case 'name':
                return row.name.toString().toUpperCase().startsWith(text.toString().toUpperCase())
                    break;
                case 'type':
                return row.type.type.toString().toUpperCase().includes(text.toString().toUpperCase())
                    break;
                case 'status':
                return row.status.status.toString().toUpperCase().startsWith(text.toString().toUpperCase())
                    break;
            }      
        })
    }
            ...
            Swal.fire({
                ...
                if (data.value) {
                    ...
                    $.ajax({
                        ...
                        url: "/products/" + row
                        .id, //URL para el controlador con el Id del producto que quiero eliminar
                        success: function (response) {
                            ...
                            $tabla_Productos.bootstrapTable(
                                "refresh") //Actualizo la tabla con los nuevos datos
                        },
                        error: function (error) {
                            console.log(
                                error) //Presenta por consola cualquier error si es que existiera
                        }
                    });
                }
            });
